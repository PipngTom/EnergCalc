    ...
    return (
        <Fragment>
            ...
            {izvestaj && (<Fragment><td>{(enerCalc(building).Qhint - rez.Qhint).toFixed(0)}</td><td>{((enerCalc(building).Qhint - rez.Qhint) * 0.039).toFixed(0)}</td>
                <td>{rez.klasa}</td><td>{(sumUnTrans + sumTrans).toFixed(0)}</td><td>{enerCalc(building).Qhint !== rez.Qhint && ((sumUnTrans + sumTrans) / ((enerCalc(building).Qhint - rez.Qhint) * 0.039)).toFixed(2)}</td><td>{(rez.kw / 1000).toFixed(1)}</td></Fragment>)}
        </Fragment>
    );
};
